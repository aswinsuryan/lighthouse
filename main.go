...
import (
	...
	"github.com/submariner-io/lighthouse/pkg/controllers/service"
...
// retrieve the Kubernetes cluster client from outside of the cluster
func getLightHouseClient() lighhouseset.Interface {
	// construct the path to resolve to `~/.kube/config`
	kubeConfigPath := os.Getenv("HOME") + "/.kube/config"
	...
	// generate the client based off of the config
	client, err := lighhouseset.NewForConfig(config)
	if err != nil {
		fmt.Println("getClusterConfig: %v", err)
	}
...
func main() {
	...
	lighthouseClient := getLightHouseClient()
	...
	informer := cache.NewSharedIndexInformer(
		...
		&api_v1.Service{}, // the target type (Pod)
	...
	controller := service.ServiceController{
		...
		Handler: &service.ServiceHandler{
			LighthouseClientset: lighthouseClient},
	}
